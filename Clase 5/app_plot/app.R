...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            colourInput('colorLine', 'Select Line colour', value= 'red'),
...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        p1 <- ggplot(house_prices, aes(x = log10_size)) + 
            geom_histogram(bins = bins, color = input$color, fill = input$colorLine) +
            labs(x = "log10 living space (square feet)", title = "House size") +
            themes[[input$theme]]
        p2 <- ggplot(house_prices, aes(x = log10_price)) + 
            geom_histogram(bins = input$bins2, color = input$color, fill = input$colorLine) +
            labs(x = "log10 living space (square feet)", title = "House price")
        p1 + p2
    })
}
